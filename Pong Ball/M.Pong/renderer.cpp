internal void
clear_screen(rafi color) {
	unsigned int* pixel = (rafi*)render_state.memory;
...
internal void
draw_rect_in_pixels(int x0, int y0, int x1, int y1, rafi color) {
	...
	for (int y = y0; y < y1; y++) {
		rafi* pixel = (rafi*)render_state.memory + x0 + y*render_state.width;
		for (int x = x0; x < x1; x++) {
			*pixel++ = color;
		}
	}
...
internal void
draw_arena_borders(float arena_x, float arena_y, rafi color) {
...
internal void
draw_rect(float x, float y, float half_size_x, float half_size_y, rafi color) {
...
internal void
draw_text(const char *text, float x, float y, float size, rafi color) {
...
internal void
draw_number(int number, float x, float y, float size, rafi color) {
	float half_size = size * .5f;
